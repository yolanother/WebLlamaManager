...
| `/v1/responses` | POST | Synchronous Responses, or retained execution with `background: true` |
| `/v1/responses/:id` | GET | Retrieve a background Response as JSON or resume a stream |
| `/v1/responses/:id/cancel` | POST | Idempotently cancel a background Response |
...
and give Distrobox only that filename after removing the token from Distrobox's
inherited host environment. The raw token is never placed in a service
definition or process command line. After rotating a token, update it in
Settings (preferred), or update the environment fallback and rerun `./install.sh`
...
Use `llama_chat` for blocking work expected to finish within the client and proxy
budget. Use `submit_response` when later retrieval or resumable streaming is
needed; `get_response` can poll JSON for either background mode and can resume
SSE only when the Response was originally created with `stream: true`. See the
...
- [Background Responses Guide](docs/Guides/AsyncInference.md) - Choosing sync versus background, retrieval, cancellation, replay, prepared append, and limits
- [Background Responses Design](docs/Designs/AsyncInferenceJobs.md) - OpenAI-compatible background Response and manager-extension architecture
