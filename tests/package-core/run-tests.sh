...
test_packaged_service_uses_declared_offline_node_runtime() {
  ...
  # Rootless Podman needs to create the engine container's cgroup; a read-only
  # /sys/fs/cgroup makes every engine start fail with podman exit 125.
  assert_contains "service can write the container cgroup" "$service" "ProtectControlGroups=no"
