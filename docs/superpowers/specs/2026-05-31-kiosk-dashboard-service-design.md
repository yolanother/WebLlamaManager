...
`uninstall`, and `restart` subcommands. It is **deliberately not wired into the
main `install.sh` / `uninstall.sh`** — not every operator wants the UI or wants
their desktop environment modified, so the kiosk is purely opt-in.
...
4. **Bring the kiosk up now** (unless `--no-start`): restart the display manager
